...
if (!function_exists('homeyHourlyGenerateMonth')) {
    function homeyHourlyGenerateMonth($numberOfMonths, $unixMonth, $currentMonth, $currentYear)
    {
        ...
        $instances = get_post_meta($post->ID, '_listing_calendar_instances', true); // Fetch instances
        ...
        for ($day = 1; $day <= $daysInMonth; ++$day) {
            ...
            // Check if the date is in the instances array and all values are blocked
            $is_blocked = false;
            if (!empty($instances) && is_array($instances)) {
                foreach ($instances as $instance) {
                    $selected_dates = explode(',', $instance['selected_dates']);
                    if (in_array(date('Y-m-d', $timestamp), $selected_dates)) {
                        if ($instance['amenity'] === 'blocked' && $instance['sleeping'] === 'blocked' && $instance['gservice'] === 'blocked') {
                            $is_blocked = true;
                            break;
                        }
                    }
            ...
            if ($is_blocked) {
                $calendar_day_class = 'day-disabled homey-not-available-for-booking';

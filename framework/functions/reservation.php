...
if (!function_exists('homey_confirm_reservation')) {
    function homey_confirm_reservation()
    {
        ...
        $start_hour = $reservation_meta['start_hour'];
        $booking_start_time = date(homey_time_format(), strtotime($start_hour));
        $end_hour = $reservation_meta['end_hour'];
        $booking_end_time = date(homey_time_format(), strtotime($end_hour));

        $total_amount = floatval($reservation_meta['total']);
        $total_amount_cents = intval(round($total_amount * 100));

        $services_fee = doubleval($reservation_meta['services_fee']);

        $occ_tax = floatval($reservation_meta['occ_tax_amount']);
        $state_tax = floatval($reservation_meta['total_state_tax']);

        $host_stripe_account_id = get_user_meta($userID, 'stripe_account_id', true);
        $card_id = get_post_meta($reservation_id, 'selected_card_id', true);
        $customer_id = get_user_meta($listing_renter, 'stripe_customer_id', true);
        $total_earning_array = get_user_meta($userID, 'host_total_earning', true);
        ...
        $host_remaining_fee = 0.0;
        $host_remaining_credit = 0.0;
        $host_fee = floatval($host_amount * 0.15);
        $total_referral_credits = floatval(get_user_meta($userID, 'referral_credit', true));

        if ($host_fee >= $total_referral_credits) {
            $host_remaining_fee = $host_fee - $total_referral_credits;
        } else {
            $host_remaining_credit = $total_referral_credits - $host_fee;
        }

        $total_host_amount = $host_amount - $host_remaining_fee;
        ...
        try {
            ...
            if ($host_fee >= $total_referral_credits) {
                update_user_meta($userID, 'referral_credit', 0);
                update_user_meta($userID, 'used_referral_credit', $total_referral_credits);
            } else {
                update_user_meta($userID, 'referral_credit', $host_remaining_credit);
                update_user_meta($userID, 'used_referral_credit', $host_fee);
            }
            ...
            update_user_meta($userID, 'host_total_earning', $total_earning);
            $current_time = current_time('mysql');
            update_post_meta($reservation_id, 'earning_transfer_date', $current_time);
        ...
        if ($no_upfront == 'no_upfront') {
            ...
            echo json_encode(
                array(
                    'success' => true,
                    'message' => 'Reservation is booked. Payment processed successfully and funds transferred.'
                )
            );
        } else {
            // Set reservation status from under_review to available
            ...
            // Send message on booking confirmation
            $message = "Backyard Lease update: Reservation - Reservation ID: " . $reservation_id . " has booked now.";
            ...
            $notification_title = 'Reservation Request: #' . $reservation_id . ' has been confirmed.';
            $notification_content = 'Reservation Request: #' . $reservation_id . ' has been confirmed by the host.';
            ...
            $notification_title = 'Booking Request: #' . $reservation_id . ' has been booked.';
            $notification_content = 'Booking Request: #' . $reservation_id . ' has been booked.';
            $notification_link = home_url('/reservations/?reservation_detail=' . $reservation_id);
            ...
            if (isset($notification_settings_guest['sms']) && $notification_settings_guest['sms']) {
                if (!empty($guest_phone_number)) {
                    $guest_message = 'BACKYARD LEASE: ' . $listing_title . ' is now booked! ' . $guest_name . ' is now confirmed for ' . $booking_date . ' from ' . $booking_start_time . ' to ' . $booking_end_time . ". See details here\n" . $reservation_url;
            ...
            if (isset($notification_settings_guest['email']) && $notification_settings_guest['email']) {
                ...
                $subject = $host_name . ' just confirmed your booking request!';
              ...
              <div style="font-family: \'Oswald\', sans-serif;text-align: center; padding: 20px; margin: 0 auto; max-width: 400px;">
                  ...
                  <p style="font-size: 14px; color: #0072ff; font-weight: 600; margin-top: 20px;">
                    ' . $host_name . ' just confirmed your</br>booking request!
                  </p>
...
if (!function_exists('homey_decline_reservation')) {
    function homey_decline_reservation()
    {
        ...
        // Send message on booking decline
        $message = "Backyard Lease update: Reservation - Reservation ID: " . $reservation_id . " has been declined.";
        send_booking_message($reservation_id, $listing_owner, $listing_renter, $message);

        // Save the booking confirmed notification to the Host
        $notification_title = 'Booking Request: #' . $reservation_id . ' has been declined.';
        $notification_content = 'Booking Request: #' . $reservation_id . ' has been declined by the host.';
        save_booking_notification($listing_owner, $notification_title, $notification_content);

        // Save the booking confirmed notification to the Guest
        $notification_title = 'Reservation Request: #' . $reservation_id . ' has been declined.';
        $notification_content = 'Reservation Request: #' . $reservation_id . ' has been declined by the host.';
        save_booking_notification($listing_renter, $notification_title, $notification_content);
...
if (!function_exists('homey_cancelled_reservation')) {
    function homey_cancelled_reservation()
    {
        ...
        $notification_content = 'Booking Request: #' . $reservation_id . ' has been cancelled by the host.';
        save_booking_notification($listing_owner, $notification_title, $notification_content);

        // Save the booking confirmed notification to the Guest
        $notification_title = 'Reservation Request: #' . $reservation_id . ' has been cancelled.';
        $notification_content = 'Reservation Request: #' . $reservation_id . ' has been cancelled by the host.';
        save_booking_notification($listing_renter, $notification_title, $notification_content);
    ...
    function homey_get_reservation_notification($status, $reservation_id = null)
    {
        ...
        if (homey_listing_guest($reservation_id)) {
            ...
            if ($status == 'under_review') {
                ...
                $notification = '<div class="alert alert-info alert-dismissible" role="alert">
                                <button type="button" class="close" data-hide="alert" aria-label="Close"><i class="fa fa-close"></i></button>
                                ' . esc_html__('Hooray! Your reservation has been completed.', 'homey') . '
                            </div>';
            }
        } else {
            if ($status == 'under_review') {
                ...
                $notification = '<div class="alert alert-info alert-dismissible" role="alert">
                                <button type="button" class="close" data-hide="alert" aria-label="Close"><i class="fa fa-close"></i></button>
                                ' . esc_html__('Hooray! Your booking has been completed.', 'homey') . '
                            </div>';
            }
        }
    ...
    function homey_get_reservation_action($status, $upfront_payment, $payment_link, $ID, $class)
    {
        ...
        if (homey_listing_guest($ID)) {
            ...
            if ($status == 'under_review') {
                ...
                $action = '<button data-reservation_id="' . intval($ID) . '" class="set-reservation-complete btn btn-success ' . esc_attr($class) . '">' . esc_html__('Set Status Complete', 'homey') . '</button>';
                $action .= '<button class="btn btn-grey-light btn-half-width" data-toggle="collapse" id="cancel-reservation-btn" data-target="#cancel-reservation" aria-expanded="false" aria-controls="collapseExample">' . esc_html__('Cancel', 'homey') . '</button>';
            } elseif ($status == 'waiting_host_payment_verification') {
                $action = '<a href="#" data-id="' . intval($ID) . '" class="mark-as-paid btn btn-success ' . esc_attr($class) . '">' . esc_html__('Payment Received? Mark as Paid', 'homey') . '</a>';
            }

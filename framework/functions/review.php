...
if (!function_exists('homey_add_review')) {
    function homey_add_review()
    {
        ...
        // Save the New Review notification to the host
        $adventure_title = get_the_title($review_listing_id);
        $notification_title = 'New review added to your Backyard Adventure: ' . $adventure_title;
        $notification_content = 'New review added to your Backyard Adventure: ' . $adventure_title;
        save_booking_notification($review_listing_owner_id, $notification_title, $notification_content);
...
add_action('wp_ajax_nopriv_homey_add_guide_review', 'homey_add_guide_review');
add_action('wp_ajax_homey_add_guide_review', 'homey_add_guide_review');
if (!function_exists('homey_add_guide_review')) {
    function homey_add_guide_review()
    {
        global $current_user;
        $current_user = wp_get_current_user();
        $userID       = $current_user->ID;
        ...
        $review_reservation_id = intval($_POST['review_guide_reservation_id']);
        $rating = intval($_POST['guide_rating']);
        $review_content = $_POST['guide_review_content'];
        $review_action = $_POST['guide_review_action'];
        ...
        $review_owner_id = $userID;
        $review_listing_id = get_post_meta($review_reservation_id, 'reservation_listing_id_for_guide', true);
        $review_listing_owner_id = get_post_meta($review_reservation_id, 'listing_owner', true);
        $update_review_id = get_post_meta($review_reservation_id, 'guide_review_id', true);

        $title = esc_html__('Review', 'homey');
        //check security
        $nonce = $_REQUEST['guide_review_security'];
        if (! wp_verify_nonce($nonce, 'review-security-nonce')) {
        ...
        update_post_meta($review_id, 'reservation_listing_id_for_guide', $review_listing_id);
        update_post_meta($review_id, 'listing_owner_id', $review_listing_owner_id);
        update_post_meta($review_id, 'reviewer_id', $review_owner_id);
        update_post_meta($review_reservation_id, 'guide_review_id', $review_id);
        update_post_meta($review_id, 'review_guide_reservation_id', $review_reservation_id);
        update_post_meta($review_id, 'homey_rating', $rating);
        update_post_meta($review_id, 'service_type', $service_type);
        update_post_meta($review_id, 'guide_review_content', $review_content);

        //if user is not listing owner then ratting should be added - zk
        if ($userID != $review_listing_owner_id) {
            homey_add_guide_listing_rating($review_listing_id);
        }
        //if user is not listing owner then ratting should be added - zk
...
if (!function_exists('homey_add_guide_listing_rating')) {
    function homey_add_guide_listing_rating($listing_id)
    {
        $args = array(
            'post_type'   => 'homey_review',
            'meta_key' => 'reservation_listing_id_for_guide',
            'meta_value' => $listing_id,
            'post_status' => 'publish',
        );
...
if (!function_exists('homey_get_host_guide_reviews')) {
    function homey_get_host_guide_reviews($author_id)
    {

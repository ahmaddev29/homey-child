  ...
  if (typeof Homey_Listing !== "undefined") {
    ...
    function homey_hourly_availability_calendar_dash() {
      ...
      var calendarEl = document.getElementById(
        "homey_hourly_calendar_edit_listing"
      );
      ...
      var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventLimit: true,
        unselectAuto: false,
        direction: homey_is_rtl ? "rtl" : "ltr",
        headerToolbar: {
          left: "returnToMonth",
          center: "title",
    ...
    var listing_gallery_images = function () {
      ...
      var plup_uploader = new plupload.Uploader({
        ...
        url:
          ajaxurl +
          "?action=homey_listing_gallery_upload&verify_nonce=" +
          verify_nonce,
        filters: {
          mime_types: [
      ...
      plup_uploader.bind("FilesAdded", function (up, files) {
        ...
        plupload.each(files, function (file) {
          homey_thumbs +=
            ...
            "</div>";
          homey_thumbs +=
            '<progress id="thumb-holder-' +
            file.id +
            '" value="0" max="100" class="progress-bar">' +
            "" +
            "</progress>";
        });
        document.getElementById("homey_gallery_container").innerHTML +=
          homey_thumbs;
        up.refresh();
        plup_uploader.start();
      });
      ...
      plup_uploader.bind("FileUploaded", function (up, file, ajax_response) {
        ...
        if (response.success) {
          ...
          var firsturl = $(".icon-featured").first().attr("data-thumb");
          $(".upload-view-media .media-image img").attr("src", firsturl);
        } else {
          var gallery_thumbnail =
            '<span class="error-message">' + response.reason + "</span>";
    ...
    */
    ...
    var thread_message_attachment = function () {
      ...
      var uploader = new plupload.Uploader({
        ...
        url:
          ajaxurl +
          "?action=homey_message_attacment_upload&verify_nonce=" +
          verify_nonce,
        filters: {
          max_file_size: "20m",
      ...
      uploader.bind("FilesAdded", function (up, files) {
        ...
        document.getElementById("listing-thumbs-container").innerHTML +=
          listingThumb;
        up.refresh();
        uploader.start();
      });
      ...
      $(".available, .unavailable").on("click", function () {
        ...
        $.ajax({
          ...
          beforeSend: function () {
            $this.children("i").remove();
            $this.prepend(
              '<i class="icon-center ' + process_loader_spinner + '"></i>'
            );
          },
          success: function (data) {
            if (data.success) {
     ...
     * -------------------------------------------------------------------------*/
    $("#save_as_drafts").click(function () {
      var $form = $("#submit_listing_form");
      var save_as_draft = $("#save_as_drafts");
        ...
        success: function (response) {
          if (response.success) {
            ...
            $("#submit_listing_form").prepend(
              '<input type="hidden" name="draft_listing_id" value="' +
                response.listing_id +
                '">'
            );
            var previewUrl =
              "https://backyardlease.flywheelsites.com/?post_type=listing&p=" +
              response.listing_id;
    ...
    $("#add_equipment_row").click(function () {
      var newRow =
        '<div class="more_extra_services_wrap">' +
        ...
        '<input type="text" name="equipment_price[]" class="form-control" placeholder="Enter price - only digits">' +
        "</div>" +
        "</div>" +
        '<div class="col-sm-4 col-xs-12">' +
        '<div class="form-group">' +
        ...
        '<option value="per_guest_fee">Per Guest</option>' +
        "</select>" +
        "</div>" +
        "</div>" +
        "</div>" +
      ...
      $("#more_equipments_rows_main").append(newRow);
      $(".type-select-picker").selectpicker("refresh");
      removeEquipmentRow();
    });

    var removeEquipmentRow = function () {
      $(".remove-equipment-row").on("click", function (event) {
        event.preventDefault();
        var $this = $(this);
        $this.closest(".more_extra_services_wrap").remove();
      });
    ...
    /*--------------------------------------------------------------------------
     *  New listing data save in local storage on continue button click
     * -------------------------------------------------------------------------*/

    if (typeof Storage !== "undefined") {
      $(".btn-step-next").on("click", function () {
        var currentStep = $("#submit_listing_form .form-step.active").data(
          "step"
        );
      ...
      function saveFormData(step) {
        ...
        $(
          '.form-step[data-step="' +
            step +
            '"] :input, .form-step[data-step="' +
            step +
            '"] textarea, .form-step[data-step="' +
            step +
            '"] select'
        ).each(function () {
        ...
        localStorage.setItem(
          "listing_form_step_" + step,
          JSON.stringify(formData)
        );
      }

      function clearFormData(stepKey) {
        localStorage.removeItem(stepKey);
      }

      // Clear form data when clicking the new listing button
      $(document).on("click", ".new-listing-button", function () {
        clearAllListingSteps();
      });

      // Clear form data when submitting the listing form
      $(document).on("submit", "#submit_listing_form", function (e) {
        clearAllListingSteps();
      });

      // Reusable function to clear all steps
      function clearAllListingSteps() {
        var steps = [
          "information",
          "pricing",
          "media",
        ...
        steps.forEach(function (step) {
          clearFormData("listing_form_step_" + step);
        });
      }
      ...
      function restoreFormData() {
        $(".form-step").each(function () {
          ...
          var storedData = localStorage.getItem("listing_form_step_" + step);

          if (storedData) {
            storedData = JSON.parse(storedData);

            for (var field in storedData) {
              var inputField = $('[name="' + field + '"]');

              if (inputField.length > 0) {
                inputField.val(storedData[field]);
        ...
        $(".form-step :input, .form-step textarea, .form-step select").trigger(
          "change"
        );
      }
      ...
      $(document).on(
        "input change",
        "#submit_listing_form .form-step :input, #submit_listing_form .form-step select, #submit_listing_form .form-step textarea",
        function () {
          var currentStep = $("#submit_listing_form .form-step.active").data(
            "step"
          );

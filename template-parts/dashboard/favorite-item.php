...
$have_sleeping_accomodations = get_field('have_sleeping_accomodations', get_the_ID());
$have_guided_service = get_field('have_guided_service', get_the_ID());

if($have_sleeping_accomodations == 'no') {
    $accomodation_allow = 'fa fa-times';
} else {
    $accomodation_allow = 'fa fa-check';
}

if($have_guided_service == 'no_guide_needed') {
    $guided_allow = 'fa fa-times';
} else {
    $guided_allow = 'fa fa-check';
}
...
<tr>
    ...
    <td data-label="<?php echo esc_attr($homey_local['price_label']) ;?>">
        ...
        <td data-label="<?php echo homey_option('glc_bedrooms_label') ;?>"><i class="<?php echo esc_attr($accomodation_allow); ?>" aria-hidden="true"></i></td>

        <td data-label="<?php echo homey_option('glc_baths_label') ;?>"><i class="<?php echo esc_attr($guided_allow); ?>" aria-hidden="true"></i></td>

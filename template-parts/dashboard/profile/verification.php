        ...
        <div class="row">
			...
			<?php } else { ?>
			    <div class="not-verified">
                    <span class="btn btn-full-width" href="#"><i class="fa fa-times-circle" aria-hidden="true"></i> <?php esc_html_e('Not Verified', 'homey'); ?></span>
                </div>
				
				<form method="post" action="">
                <input type="hidden" name="resend_verification" value="1">
                <input type="submit" class="btn btn-full-width" value="<?php esc_html_e('Resend Email', 'homey'); ?>">
                </form>
                <?php

                if (isset($_POST['resend_verification']) && $_POST['resend_verification'] == 1) {
                send_verification_email($user_id, get_the_author_meta('user_email', $user_id));
                echo '<p>' . esc_html__('Verification email resent! Please check your email.', 'homey') . '</p>';
                }
			    }  
				?>
            </div>
...
<div class="block">
    <div class="block-title">
        <div class="block-left">
			...
			<p><?php esc_html_e('You DO NOT have to upload your ID to list or post on the Backyard Lease platform. Uploading your ID is confidential and will never be shared. This additional layer of security is needed only to Verify your authenticity and gives guests the assurance that you are you – and not someone pretending to be you. Hosts who are verified and more likely to get booked.', 'homey'); ?></p>
            <?php } ?>

            <?php if (homey_is_renter()) { ?>
            <p><?php esc_html_e('You DO NOT have to upload your ID to list or post on the Backyard Lease platform. Uploading your ID is confidential and will never be shared. This additional layer of security is needed only to Verify your authenticity and gives hosts the assurance that you are you – and not someone pretending to be you. Guests who are verified and more likely to get approved bookings.', 'homey'); ?></p>
            <?php } ?>
        </div>
    </div>
    <div class="block-body">
            ...
            <div class="col-sm-3">
                ...
                <?php } else { ?>
                        <?php if($is_doc_verified_request) { ?>
                            <div class="not-verified">
                                <span class="btn btn-full-width" href="#"> <?php esc_html_e('Waiting for approval', 'homey'); ?></span>
                            </div>
                        <?php } ?>

...
$total_price = doubleval($item_meta['total']);
...
if (true === homey_is_host_by_name($listing_author['name'])) :
?>

    <tr class="booking-from-another-host">
        <td colspan="11">Backyard Lease Host is booking your amenity</td>
    </tr><?php
        endif; ?>
<tr <?php if (true === homey_is_host_by_name($listing_author['name'])) : ?>style="border: 1px solid;" <?php endif; ?> class="<?php echo esc_attr($is_read); ?>">
    ...
    <td data-label="<?php echo esc_attr($homey_local['id_label']); ?>">
        ...
        if ($wc_order_id > 0) echo 'wc#' . $wc_order_id; ?>
    </td>
    ...
    <td data-label="<?php echo esc_attr($homey_local['address']); ?>">
        ...
        <?php
        if ($reservation_status == 'under_review' || $reservation_status == 'declined' || $reservation_status == 'cancelled') {
            if (!empty($listing_city || $listing_state)) {
        ?>
                <address><?php echo esc_attr($listing_city) . ', ' . esc_attr($listing_state); ?></address>
        <?php }
        } ?>
        <?php
        if ($reservation_status == 'booked' || $reservation_status == 'available') {
    ...
    <td data-label="<?php echo esc_attr($homey_local['subtotal_label']); ?>">
        <strong><?php echo homey_formatted_price($total_price); ?></strong>
    </td>

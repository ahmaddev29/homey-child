                    ...
                    <div class="col-lg-12 col-md-12 col-sm-12">
                        <div class="dashboard-area">
                            ...
                            <?php if (isset($_GET['edit_listing']) && (isset($_GET['featured']) && $_GET['featured'] == 1)) { 
							    ...
							    $featured_listings_count = get_featured_listings_count();
								if ($featured_listings_count < 28) {
							...
							?>
								...
								<?php } else { 
									...
									wp_schedule_single_event( time() + $time_difference, 'make_listing_featured_event', array( $listing_id ) );
									update_post_meta( $listing_id, 'homey_queued_featured', 1 );
									$start_time = current_time( 'timestamp' ) + $time_difference;
									update_post_meta( $listing_id, 'homey_queued_featured_time', $start_time );
									// Calculate expiry time (5 days from now)
                                    $queued_expiry_time = strtotime('+5 days', $start_time);
                                    update_post_meta($listing_id, 'homey_featured_queued_expiry', $queued_expiry_time);
								?>
									<div class="alert alert-info alert-dismissible" role="alert">
                                        <button type="button" class="close" data-hide="alert" aria-label="Close"><i class="fa fa-close"></i></button>
                                        <?php echo "Your listing will become featured once space is available. You can check it on calendar when your listing will become featured"; ?>
                                    </div>
								<?php } ?>
                            <?php } ?>
